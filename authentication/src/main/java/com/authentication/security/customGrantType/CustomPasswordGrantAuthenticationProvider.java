package com.authentication.security.customGrantType;
...
import com.authentication.domain.oauth.Authorization;
import com.authentication.security.ResourceOwner;
...
public class CustomPasswordGrantAuthenticationProvider implements AuthenticationProvider {
	...
	public CustomPasswordGrantAuthenticationProvider(OAuth2TokenGenerator<? extends OAuth2Token> tokenGenerator, UserDetailsService userDetailsService,
	...
	public Authentication authenticate(Authentication authentication) throws AuthenticationException {
		...
		User u = new User(user.getUsername(), user.getPassword(), user.isEnabled(), user.isAccountNonExpired(), user.isCredentialsNonExpired(), user.isAccountNonLocked(), user.getAuthorities());
		...
		// ----- Access Token -----
		Authorization authorization_ = this.authorizationService.findByUsernameAndGrantType(user.getUsername(), CUSTOM_PASSWORD);
		...
		if(authorization_ != null) {
			if(authorization_.getAccessTokenExpiresAt().isBefore(Instant.now())) {
				this.authorizationService.remove(OAuth2Authorization.withRegisteredClient(registeredClient).id(authorization_.getId()).principalName(clientPrincipal.getName()).authorizedScopes(authorizedScopes).authorizationGrantType(new AuthorizationGrantType(CUSTOM_PASSWORD)).build());
				generatedAccessToken = this.tokenGenerator.generate(tokenContext);
			} else {
				generatedAccessToken = new OAuth2AccessToken(OAuth2AccessToken.TokenType.BEARER, authorization_.getAccessTokenValue(), Instant.now(), authorization_.getAccessTokenExpiresAt(), authorizedScopes);
			}
		} else {
			generatedAccessToken = this.tokenGenerator.generate(tokenContext);
		}
		...
		if (registeredClient.getAuthorizationGrantTypes().contains(AuthorizationGrantType.REFRESH_TOKEN)
			...
			if(authorization_ == null || authorization_.getRefreshTokenExpiresAt().isBefore(Instant.now())) {
				...
				refreshToken = new OAuth2RefreshToken(authorization_.getRefreshTokenValue(), Instant.now(), authorization_.getRefreshTokenExpiresAt());
			}
		...
		/*accessToken = new OAuth2AccessToken(OAuth2AccessToken.TokenType.BEARER,
				generatedAccessToken.getTokenValue(), Instant.now(),
				generatedAccessToken.getExpiresAt(), authorizedScopes);*/
